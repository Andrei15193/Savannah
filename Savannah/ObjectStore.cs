...
namespace Savannah
{
    public class ObjectStore
    {
        ...
        private sealed class InsertOperationContext
        {
        ...
        public async Task ExecuteAsync(ObjectStoreOperation operation, CancellationToken cancellationToken)
        {
            ...
            switch (operation.OperationType)
            {
                case ObjectStoreOperationType.Insert:
                    await _InsertAsync(operation.Object, cancellationToken);
                    break;
        ...
        private static void _Validate(ObjectMetadata metadata)
        {
            if (metadata.PartitionKeyProperty?.GetMethod == null)
                throw new InvalidOperationException("The given object must expose a readable PartitionKey property of type string.");
            if (metadata.RowKeyProperty?.GetMethod == null)
        ...
        private async Task _InsertAsync(object @object, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                using (var temporaryFileStream = await temporaryFile.OpenStreamForWriteAsync().ConfigureAwait(false))
                {
                    ...
                    {
                        var context = new InsertOperationContext
                        {
        ...
        private async Task _InsertAsync(InsertOperationContext context, CancellationToken cancellationToken)
        {
        ...
        private async Task _InsertInBucketAsync(InsertOperationContext context, CancellationToken cancellationToken)
        {
            var inserted = false;
            do
        ...
        private async Task _InsertInPartitionAsync(InsertOperationContext context, CancellationToken cancellationToken)
        {
#if DEBUG
            if (!context.XmlReader.IsOnPartitionElement())
        ...
        private async Task _WritePartitionAsync(InsertOperationContext context, CancellationToken cancellationToken)
        {
            await context.XmlWriter.WritePartitionStartElementAsync(cancellationToken).ConfigureAwait(false);
